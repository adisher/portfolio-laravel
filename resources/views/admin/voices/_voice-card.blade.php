...
@php $conf = $v->meta['confidence'] ?? null; @endphp
<div class="border border-gray-200 dark:border-gray-700 rounded-lg p-3">
    <div class="flex gap-3">
        <div class="flex-shrink-0 w-16">
            ...
            <form method="POST" action="{{ route('admin.voices.update', $v) }}" class="text-center mt-1">
                @csrf @method('PATCH')
        ...
        <div class="flex-1 min-w-0">
            ...
            <p class="text-xs text-gray-400 mt-1">
                ...
                @if($conf) &middot; <span class="{{ $conf === 'high' ? 'text-green-600' : ($conf === 'low' ? 'text-red-400' : 'text-amber-500') }}">{{ $conf }} confidence</span>@endif
                @if($v->meta['engine'] ?? null) &middot; via {{ $v->meta['engine'] }}@endif
            </p>
            @if($v->meta['note'] ?? null)
            <p class="text-[11px] text-gray-400 mt-1 italic">{{ $v->meta['note'] }}</p>
            @endif
            <div class="flex flex-wrap items-center gap-x-4 gap-y-2 mt-2">
                <form method="POST" action="{{ route('admin.voices.update', $v) }}">
                    @csrf @method('PATCH')
                    <input type="hidden" name="status" value="{{ $v->status === 'approved' ? 'candidate' : 'approved' }}">
                    <button class="text-xs {{ $v->status === 'approved' ? 'text-gray-500' : 'text-green-600 font-medium' }} hover:underline">
                        {{ $v->status === 'approved' ? 'Unapprove' : 'Approve' }}
                    </button>
                </form>

                {{-- Screenshot the source, copy it, then paste here --}}
                <form method="POST" action="{{ route('admin.voices.update', $v) }}" enctype="multipart/form-data" class="voice-upload flex items-center gap-2">
                    @csrf @method('PATCH')
                ...
                <form method="POST" action="{{ route('admin.voices.destroy', $v) }}" onsubmit="return confirm('Remove this voice?')">
                    @csrf @method('DELETE')
