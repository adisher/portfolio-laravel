...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Edit Tournament</h1>
    </div>

    <form method="POST" action="{{ route('admin.sports.tournaments.update', $tournament) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
                ...
                <div class="admin-card">
                    ...
                    <div class="px-6 py-4 space-y-4">
                        ...
                        <div>
                            ...
                            <select id="sport_id" name="sport_id" required
                                ...
                                @foreach($sports as $sport)
                                    <option value="{{ $sport->id }}" @selected(old('sport_id', $tournament->sport_id) == $sport->id)>
                                        {{ $sport->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="text" id="season" name="season" value="{{ old('season', $tournament->season) }}"
                        ...
                        <div>
                            ...
                            <input type="text" id="country" name="country" value="{{ old('country', $tournament->country) }}"
                        ...
                        @if($tournament->logo_url)
                            <div class="flex flex-col items-center">
                                <img src="{{ $tournament->logo_url }}" alt="{{ $tournament->name }}"
                                     class="w-24 h-24 rounded-lg object-cover border border-gray-200 dark:border-gray-700 mb-2">
                                <p class="text-xs text-gray-500 dark:text-gray-400">Current logo</p>
                            </div>
                        @endif

                        <div class="flex items-center justify-center w-full">
                            <label for="logo" class="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg cursor-pointer hover:border-gray-400 dark:hover:border-gray-500 transition-colors bg-gray-50 dark:bg-gray-800">
                                <div class="flex flex-col items-center justify-center py-4">
                                    ...
                                    <p class="text-sm text-gray-500 dark:text-gray-400">
                                        {{ $tournament->logo ? 'Upload new logo' : 'Click to upload logo' }}
                                    </p>
                    ...
                    <div class="px-6 py-4 space-y-4">
                        ...
                        <label class="flex items-center gap-3 cursor-pointer">
                            ...
                            <input type="checkbox" name="is_featured" value="1"
                                   ...
                                   @checked(old('is_featured', $tournament->is_featured))>
                            <div>
                        ...
                        <label class="flex items-center gap-3 cursor-pointer">
                            ...
                            <input type="checkbox" name="is_active" value="1"
                                   class="w-4 h-4 rounded border-gray-300 dark:border-gray-600 text-blue-600 focus:ring-blue-500 dark:bg-gray-700"
                                   @checked(old('is_active', $tournament->is_active))>
                            <div>
