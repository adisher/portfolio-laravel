...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Solution</h1>
</div>
...
<form action="{{ route('admin.solutions.update', $solution) }}" method="POST">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="admin-card p-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" id="title" name="title" required value="{{ old('title', $solution->title) }}"
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="3" required
                            ...
                            class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white @error('description') border-red-500 @enderror">{{ old('description', $solution->description) }}</textarea>
                        @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="icon" name="icon" rows="4"
                            ...
                            class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white font-mono text-sm">{{ old('icon', $solution->icon) }}</textarea>
                    ...
                    <div>
                        ...
                        <input type="text" id="keywords" name="keywords"
                            value="{{ old('keywords', $solution->keywords ? implode(', ', $solution->keywords) : '') }}"
            ...
            @if($solution->icon)
            <div class="admin-card p-6">
                <h2 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Icon Preview</h2>
                <div class="flex items-center gap-4">
                    ...
                    <span class="text-sm text-gray-500 dark:text-gray-400">This is how the icon appears on the frontend.</span>
                </div>
            </div>
            @endif
        </div>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select id="accent_color" name="accent_color" required
                            class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                            <option value="teal" {{ old('accent_color', $solution->accent_color) === 'teal' ? 'selected' : '' }}>Teal</option>
                            <option value="sunset" {{ old('accent_color', $solution->accent_color) === 'sunset' ? 'selected' : '' }}>Sunset</option>
                            <option value="blend" {{ old('accent_color', $solution->accent_color) === 'blend' ? 'selected' : '' }}>Blend (Teal → Sunset)</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" id="sort_order" name="sort_order" value="{{ old('sort_order', $solution->sort_order) }}"
                            class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                    ...
                    <div class="space-y-2">
                        <label class="flex items-center">
                            <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $solution->is_featured) ? 'checked' : '' }}
                                class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                            <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">Featured on Home Page</span>
                        </label>

                        <label class="flex items-center">
                            <input type="checkbox" name="is_published" value="1" {{ old('is_published', $solution->is_published) ? 'checked' : '' }}
                                class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
